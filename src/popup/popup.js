...
async function toggleDeepWork() {
  const stored = await chrome.storage.local.get(['deepWorkMode', '_deepWorkPreviousSettings']);
  const newMode = !stored.deepWorkMode;
  const affectedSettings = ['hideHomeFeed', 'hideSidebarRecommendations', 'hideShorts', 'hideShortsSearch', 'hideShortsSearchManual', 'hideEndscreenRecommendations', 'hideComments'];

  if (newMode) {

    const previous = {};
    for (const key of affectedSettings) {
      previous[key] = stored[key] ?? false;
    }
    await chrome.storage.local.set({ _deepWorkPreviousSettings: previous });

    const updates = { deepWorkMode: true };
    for (const key of affectedSettings) {
      updates[key] = true;
    }
    ...
    const previous = stored._deepWorkPreviousSettings || {};
    const updates = { deepWorkMode: false };
    for (const key of affectedSettings) {
      updates[key] = previous[key] ?? false;
    }
    await chrome.storage.local.set(updates);
  }
  ...
  checkboxes.forEach(id => {
    const el = getEl(id);
    if (el) el.checked = newMode ? true : (stored._deepWorkPreviousSettings?.[id] ?? false);
  });
